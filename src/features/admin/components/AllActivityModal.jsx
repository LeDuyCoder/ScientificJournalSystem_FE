  ...
  return (
    <Modal show={show} onHide={onClose} centered scrollable>
      <Modal.Header className="admin-activity-modal__header">
        ...
        {/* Close button - icon X, dong bo voi cac modal khac */}
        <button type="button" className="admin-activity-modal__close" onClick={onClose} aria-label="Close">
          <Icon icon="lucide:x" />
        </button>
      </Modal.Header>
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.id} className="admin-activity-item">
                <span className="admin-activity-item__dot" style={{ backgroundColor: dotColor }} />
