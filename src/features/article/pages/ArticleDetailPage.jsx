...
export default function ArticleDetailPage() {
  ...
  const handleTitleClick = () => {
    if (!article) return;
    const paperUrl = article.source_url || article.doi_url || (article.doi ? getDoiUrl(article.doi) : `https://www.semanticscholar.org/search?q=${encodeURIComponent(article.title)}`);
    if (paperUrl) {
          ...
          <main className="article-connected-shell">
                ...
                {isRecommendedLoading ? (
                  ...
                  recommendedArticles.map((rec) => (
                    <div 
                      ...
                      onClick={() => navigate(`/articles/${rec.article_id}`)}
                    >
                      <div className="d-flex align-items-center justify-content-between mb-1">
            ...
            <aside className="article-connected-right">
              <div className="detail-pane-content px-4 py-4">
                ...
                {/* Title */}
                <h1 className="article-detail-title is-clickable" onClick={handleTitleClick} title="Open in new tab">
                  {article.title}
                  <Icon icon="lucide:external-link" width="16" className="ms-2 text-muted" />
                </h1>
