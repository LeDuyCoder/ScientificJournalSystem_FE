...
// ======================================================================
const ArticleDetailPane = ({ articleId, onTitleClick }) => {
  ...
  const fetchArticleForPane = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && response.data.success !== false) {
        ...
        throw new Error(response.data?.message || 'Không thể tải chi tiết bài báo.');
      }
    } catch (err) {
      ...
      if (err.isAxiosError && err.response && err.response.status === 404) {
        setError({ type: '404', message: 'Không tìm thấy bài báo này.' });
      } else {
        setError({ type: 'generic', message: err.message || 'Lỗi khi tải dữ liệu.' });
      }
    } finally {
      setIsLoading(false);
    }
    ...
    if (error.type === '404') {
      return (
        <div className="p-4 text-center">
          ...
          <h4 className="font-display fw-bold text-main">Bài báo không tồn tại</h4>
          <p className="text-muted-custom px-2">
            Có vẻ như chúng tôi không thể tìm thấy thông tin cho bài báo bạn đang tìm kiếm.
            Mã định danh `ID: {articleId}` có thể không hợp lệ hoặc đã bị xóa.
          </p>
          <Button variant="outline-primary" onClick={fetchArticleForPane} className="mt-3">Thử lại</Button>
          <p className="text-muted-custom mt-3">
            <small>Nếu bạn vẫn gặp lỗi, hãy kiểm tra ID của node được gửi từ biểu đồ.</small>
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="detail-pane-content px-4 py-4">
      ...
      <h1 className="article-detail-title is-clickable" onClick={() => onTitleClick(article.article_id)} title="Xem chi tiết bài báo">
        {article.title}
      </h1>
            ...
            {article.keywords.map((keyword) => {
              ...
              return (
                <span
                  ...
                  onClick={() => navigate(`/keywords/${keyword.keyword_id || ''}`)}
