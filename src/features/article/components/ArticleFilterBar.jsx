...
export default function ArticleFilterBar({ filters, updateFilters, clearFilters }) {
  ...
  const filterConfigs = [
    ...
    },
    {
      ...
      searchable: true,
      emptyLabel: 'Không tìm thấy tạp chí'
    },
    {
      id: 'topic',
      label: 'Chủ đề',
      ...
      searchable: true,
      emptyLabel: 'Không tìm thấy chủ đề'
    }
  ];
  ...
  if (currentSortValue !== 'created_at-desc') {
    activeChips.push({
      id: 'sort',
      label: 'Sort',
      value: SORT_OPTIONS.find(o => o.value === currentSortValue)?.label || 'Sắp xếp',
        ...
        <Dropdown.Menu className="article-filter-menu">
          <div className="article-filter-menu-header">
            ...
            {config.selectedLabel && (
              <button type="button" onClick={config.onClear} className="article-filter-reset-btn">
                Reset
              </button>
            )}
          </div>
          ...
          <div className="article-filter-options">
            {filteredOptions.length > 0 ? (
              ...
              <div className="article-filter-empty">{config.emptyLabel || 'Không có lựa chọn phù hợp'}</div>
            )}
          </div>
        </Dropdown.Menu>
      </Dropdown>
      ...
      <div className="article-filter-layout">
        <div className="article-filter-toolbar" aria-label="Article filters">
          ...
          <Dropdown className="article-filter-dropdown" autoClose="outside">
            ...
            <Dropdown.Menu className="article-filter-menu">
              <div className="article-filter-menu-header">
                <span>Sắp xếp bài báo</span>
                {currentSortValue !== 'created_at-desc' && (
                  <button type="button" onClick={() => handleSortChange('created_at-desc')} className="article-filter-reset-btn">
                    Reset
                  </button>
                )}
              </div>
          ...
          <Dropdown className="article-filter-dropdown" autoClose="outside">
            ...
            <Dropdown.Menu className="article-filter-menu article-more-filter-menu">
              ...
              <button
                type="button"
                onClick={() => handleSelectChange('access')(filters.selectedAccess === 'oa' ? 'all' : 'oa')}
                className={`article-toggle-row ${filters.selectedAccess === 'oa' ? 'is-active' : ''}`}
                style={{
                  width: '100%',
                  border: 0,
              ...
              >
                <Form.Check
                  type="switch"
                  checked={filters.selectedAccess === 'oa'}
                  readOnly
                  className="m-0"
                />
                <span>Chỉ hiển thị Open Access</span>
              </button>
            </Dropdown.Menu>
          </Dropdown>
        </div>
