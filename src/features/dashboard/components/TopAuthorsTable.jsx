...
function RankBadge({ rank }) {
  const cfg = {
    1: { bg: 'var(--text-main)', color: 'var(--bg-main)', icon: 'lucide:crown' },
    2: { bg: 'var(--border)', color: 'var(--text-main)', icon: null },
    3: { bg: 'var(--bg-section)', color: 'var(--text-main)', icon: null },
  }[rank] ?? { bg: 'transparent', color: 'var(--text-muted)', icon: null };

  return (
    <div
      className="d-flex align-items-center justify-content-center rounded-circle flex-shrink-0"
      style={{ width: 28, height: 28, backgroundColor: cfg.bg, color: cfg.color, fontSize: '0.75rem', fontWeight: 700 }}
    >
      ...
      {authors.map((author, i) => {
        ...
        return (
          <div
            ...
            className="d-flex d-md-grid px-4 py-3 align-items-center gap-3 font-display"
            style={{
              gridTemplateColumns: '40px 200px 1fr 80px 90px',
