...
export default function TransactionHistoryPage() {
  ...
  useEffect(() => {
    ...
    getWalletTransactions(params)
      ...
      .catch(() => {
        if (mounted) setError('Lỗi kết nối lịch sử giao dịch.');
      })
      .finally(() => {
  ...
  return (
    <div className="tx-history-page">
      ...
      <div className="tx-history-hero">
        <h1 className="tx-history-title">Lịch sử biến động coin</h1>
        <p className="tx-history-subtitle">Xem lại danh sách nạp, tiêu và hoàn coin chi tiết trong ví của bạn.</p>
      </div>

      {/* Filter Tabs */}
      <div className="tx-history-filters">
        <button type="button" className={`tx-filter-tab ${filterType === '' ? 'active' : ''}`} onClick={() => { setFilterType(''); setPage(1); }}>
          Tất cả
        </button>
        <button type="button" className={`tx-filter-tab ${filterType === 'deposit' ? 'active' : ''}`} onClick={() => { setFilterType('deposit'); setPage(1); }}>
          Nạp coin
        </button>
        <button type="button" className={`tx-filter-tab ${filterType === 'spend' ? 'active' : ''}`} onClick={() => { setFilterType('spend'); setPage(1); }}>
          Tiêu coin
        </button>
        <button type="button" className={`tx-filter-tab ${filterType === 'refund' ? 'active' : ''}`} onClick={() => { setFilterType('refund'); setPage(1); }}>
          Hoàn coin
        </button>
      </div>
      ...
      <div className="tx-history-card">
        {loading ? (
          <div className="tx-history-table-loading">Đang tải lịch sử giao dịch...</div>
        ) : error ? (
          <div className="tx-history-table-empty">{error}</div>
        ) : transactions.length === 0 ? (
          <div className="tx-history-table-empty">Không tìm thấy giao dịch nào.</div>
        ) : (
          <>
                  ...
                  {transactions.map((tx) => {
                    ...
                    return (
                      <tr key={tx.wallet_transaction_id}>
                        <td>
                          <div className="tx-history-table-desc">
                            ...
                            <div>
                              ...
                              <div className="tx-history-table-detail">{tx.description || 'Giao dịch ví coin'}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td style={{ fontWeight: 800 }}>{formatCoin(tx.balance_after)}</td>
                        <td>
